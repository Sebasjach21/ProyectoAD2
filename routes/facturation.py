...
# ==========================================
# 📥 Descarga de Facturas (REST)
# ==========================================
@bp.route("/factura/<int:id>/xml", methods=["GET"])
def descargar_xml(id):
    from services.facturacion import obtener_datos_factura, generar_xml_factura
    conn = None
    try:
        conn = get_connection()
        row = obtener_datos_factura(id, conn)
        if not row:
            return jsonify({"success": False, "message": "Factura no encontrada"}), 404
        
        email_cliente, nombre_cliente, total, productos = row
        xml_content = generar_xml_factura(id, nombre_cliente, total, productos)
...
def descargar_pdf(id):
    from services.facturacion import obtener_datos_factura, generar_pdf_factura
    conn = None
    pdf_file = None
    try:
        conn = get_connection()
        ...
        email_cliente, nombre_cliente, total, productos = row
        pdf_file = generar_pdf_factura(id, nombre_cliente, total, productos)
